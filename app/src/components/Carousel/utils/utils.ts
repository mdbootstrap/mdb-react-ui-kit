export const isVisible = (element: HTMLDivElement): boolean => {
  const elementStyle = getComputedStyle(element);
  const parentNodeStyle = getComputedStyle(element?.parentNode as Element);

  const isElementVisisble =
    elementStyle.display !== 'none' && parentNodeStyle.display !== 'none' && elementStyle.visibility !== 'hidden';

  return isElementVisisble;
};

export const getCarouselItems = (carouselElement: HTMLDivElement): HTMLDivElement[] =>
  Array.from(carouselElement?.querySelectorAll('.carousel-item'));
...
export const queueCallback = (callback: () => void, element: HTMLElement, isAnimated = true): void => {
  if (!isAnimated) {
    execute(callback);
    return;
  }
